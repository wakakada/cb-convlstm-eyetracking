...
def run_inference(video_path, model_path, output_path, use_kalman=True):
    ...
    model.eval()    # 模型设置为评估模式，会禁用Dropout、BatchNorm的统计更新等，保证推理结果稳定
    ...
    frame_buffer = []   # 滑动窗口缓冲区，用于存储连续SEQ_LEN帧预处理后的图像数据
    ...
    while True:
        # 逐帧处理视频
        ret, frame = cap.read()
        if not ret: # 循环读取视频帧，直到ret为False（视频结束）
            break

        # 预处理帧
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        resized = cv2.resize(gray, (WIDTH, HEIGHT))
        normalized = resized.astype(np.float32) / 255.0     # 图像像素值归一化
        frame_buffer.append(normalized)

        # 当缓冲区满时进行预测
        if len(frame_buffer) == SEQ_LEN:
            # 构造输入 Tensor: (1, Seq, 1, H, W)
            input_data = np.array(frame_buffer)     # 转换为(SEQ_LEN, H, W)的Numpy数组
            input_data = np.expand_dims(input_data, axis=0)  # Batch=1，增加批次维度->(1, SEQ_LEN, H, W)
            input_data = np.expand_dims(input_data, axis=2)  # Channel=1，增加通道维度->(1, SEQ_LEN, 1, H, W)
            input_tensor = torch.from_numpy(input_data).to(DEVICE).float()

            with torch.no_grad():   # 上下文禁用梯度计算，减少显存占用并加速推理
                # 输出形状: (1, Seq, 2)
                ...
                print(f"({detection_x}, {detection_y})")

                # 计算检测置信度（基于预测的稳定性），可改进成根据模型输出的方差或预测坐标的时序变化来估算
